...
object LdapSearchFactory extends Logging {
  ...
  private def createDirContext(conf: LivyConf, principal: String, password: String): InitialDirContext = {
...
class LdapSearchFactory extends DirSearchFactory with Logging {
  ...
  def getInstance(conf: LivyConf, principal: String, password: String): DirSearch = {
    try {
      val ctx = LdapSearchFactory.createDirContext(conf, principal, password)
